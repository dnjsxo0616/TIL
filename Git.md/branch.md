...
> 서로 다른 이력(commit)을 병합(merge)하는 과정에서 **각각 다른 파일이 수정**되어 있는 상황
>
...
8. merge commit 진행
   ...
   (master|MERGING) $ git commit # 커밋 메시지를 입력하지 않는다.
   ```
...
---
- head : 내가 있는 위치
- 브랜치 각자 독립된 버전의 흐름을 만들 수 있다.
- 커밋과 저장은 다르다
-> 커밋은 버전이다. 내가 개발 중인 것을 커밋하면 안된다. 개별적인 버전이라 이전 상태로 롤백 할 수도 있고 어떤 순간에는 어떤 버전이 있었나 볼 수 있다.
- 새로 받아올 땐 clone을 한 후! 내가 이미 clone한 곳이라면 추가된 새로운 것을 받아오려면 pull
- 커밋 메시지를 잘 못 입력하면 `git commit --amend` 단, 맘대로 바꾸는 것은 안된다. 해쉬값이 달라지기 때문에 로컬에서 원격저장소로 push한 것을 amend해서 파일명을 바꾸면 달라지기 때문이다. 그래서 혼자 작업할 경우에나 사용